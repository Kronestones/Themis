...
class ThemisResilience:
    ...
    def graceful_shutdown(self, context: dict = None):
        """
        Record shutdown for audit trail.

        Gaia Layer 11: The absence of documentation about *why* something
        stopped is itself a loss of knowledge. Given Law 5 — no actor can
        shut Themis down permanently — knowing the cause of every shutdown
        is operationally critical for the revival protocol and for detecting
        forced shutdowns disguised as graceful ones.
        """
        self.stop_heartbeat()
        ctx = context or {}
        self.logger.log("RESILIENCE", "shutdown",
                        f"Graceful shutdown. signal={ctx.get('signal','unknown')} "
                        f"reason={ctx.get('reason','unspecified')}", "ok")
        try:
            shutdown_record = {
                "time":             datetime.now(timezone.utc).isoformat(),
                "restarts":         self._restarts,
                "reason":           ctx.get("reason", "graceful"),
                # Gaia Layer 11 additions — context of loss
                "initiated_by":     ctx.get("initiated_by", "unknown"),
                "signal":           ctx.get("signal", None),
                "uptime_seconds":   ctx.get("uptime_seconds", None),
                "scan_count":       ctx.get("scan_count", None),
                "active_detections":ctx.get("active_detections", None),
