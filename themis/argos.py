...
class ArgosScanner:
    ...
    def _scan_wifi_surveillance(self, settings: dict) -> list:
        ...
        for network in networks:
            ...
            # Hidden network: only flag if signal is strong AND MAC matches
            # a known surveillance vendor — avoids noisy low-confidence alerts
            if not ssid and signal > -50:
                vendor = self.SURVEILLANCE_MAC_PREFIXES.get(mac_prefix)
                if vendor:
                    detections.append({
                        "lead":       "Argos",
                        "type":       "hidden_network",
                        "detail":     f"Strong hidden {vendor} network. BSSID: {bssid}",
                        "confidence": 0.70,
                        "severity":   "warning",
                        "vendor":     vendor,
                        "bssid":      bssid,
    ...
    def _arp_scan(self) -> list:
        """Get local network devices. Uses ip neigh (works unprivileged in Termux/Android)
        with arp -n as fallback for other Linux environments."""
        devices = []

        # Try ip neigh first — works without root in Termux
        try:
            r = subprocess.run(
                ["ip", "neigh", "show"],
            ...
            if r.returncode == 0:
                for line in r.stdout.splitlines():
                    ...
                    if len(parts) >= 5 and parts[3] == "lladdr":
                        devices.append({"ip": parts[0], "mac": parts[4]})
                if devices:
                    return devices
        except Exception:
            pass

        # Fallback: arp -n (standard Linux, may need root on Android)
        try:
            r = subprocess.run(
                ["arp", "-n"],
                capture_output=True, text=True, timeout=10
