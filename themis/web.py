...
def api_infrastructure():
    ...
    Accepts optional ?type= param to return one layer at a time.
    This keeps responses small and fast on Render free tier.
    All sourced from public records — EFF, DHS, ICE.gov, ACLU, journalism.
    """
    filter_type = request.args.get("type", None)
    records = get_infrastructure(filter_type=filter_type)
    return jsonify({
        "ok":      True,
        "records": records,
        "count":   len(records),
