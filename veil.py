...
class VeilAlerts:
    ...
    def process_detections(self, detections: list, settings: dict):
        ...
        # Disappearance events are NOT added — they are synthetic alerts, not
        # real signals. Adding them would create phantom entries that then
        # immediately "disappear" next scan, causing an infinite alert loop.
    ...
    def _check_disappearances(self, current_detections: list) -> list:
        ...
        for key, record in list(store.items()):
            if key not in current_keys:
                ...
                elif record["missed_scans"] > DISAPPEARANCE_GRACE_SCANS + 1:
                    # Already alerted — prune from store so it won't fire again
                    del store[key]
            else:
